...
describe('getProjectileGroundShadow', () => {
  it('projects onto the current live terrain column without mutating either input', () => {
    const terrain = terrainWithSurface([[100, 420], [101, 300]]);
    ...
    expect(first?.groundY).toBe(420);
    expect(first?.altitude).toBe(160);
  ...
  it('widens and softens monotonically with altitude, then stays capped', () => {
    const terrain = terrainWithSurface([[200, 500]]);
    const low = getProjectileGroundShadow({ x: 200, y: 480 }, terrain);
    const middle = getProjectileGroundShadow({ x: 200, y: 340 }, terrain);
  ...
  it('pins the exact bounded altitude envelope at low, middle, and cap', () => {
    const terrain = terrainWithSurface([[300, 500]]);
    const low = getProjectileGroundShadow({ x: 300, y: 500 - 1 }, terrain);
    const middle = getProjectileGroundShadow({ x: 300, y: 500 - 160 }, terrain);
    const cap = getProjectileGroundShadow({ x: 300, y: 500 - 320 }, terrain);
