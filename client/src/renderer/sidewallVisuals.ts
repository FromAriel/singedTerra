...
export function drawReflectiveSidewalls(
  ctx: CanvasRenderingContext2D,
  walls: WallMode,
  contacts: readonly Readonly<WallContactVisual>[],
  reduceMotion = false,
): void {
  if (walls !== 'reflective') return;

  ctx.save();
  ctx.globalCompositeOperation = 'screen';
  ctx.strokeStyle = 'rgba(104, 226, 255, 0.92)';
  ctx.shadowColor = 'rgba(73, 197, 255, 0.78)';
  ctx.shadowBlur = 9;
  ctx.lineWidth = 2;
  ...
  if (!reduceMotion) {
    ctx.fillStyle = 'rgba(225, 251, 255, 0.96)';
    for (const contact of contacts) {
      const life = 18;
      if (contact.age < 0 || contact.age >= life) continue;
      ctx.globalAlpha = (1 - contact.age / life) ** 2;
      const x = contact.side === 'left' ? 0 : CANVAS_WIDTH - 7;
      const halfHeight = 9 + contact.age * 1.6;
      ctx.fillRect(x, contact.y - halfHeight, 7, halfHeight * 2);
    }
  }
  ctx.restore();
}
