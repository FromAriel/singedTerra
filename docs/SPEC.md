...
- **Directional terrain bevels**: Each version-triggered terrain texture rebuild derives a bounded three-pixel bevel from the local bitmap. Upward and leftward exposure blends toward the warm rim; rightward and downward exposure blends toward a cool backdrop shadow, so hills, crater walls, caves, and overhangs retain dimensional light after deformation. Fully enclosed pixels, coverage alpha, strata, gameplay terrain, replay, and Supabase remain unchanged, and stable frames only blit the existing cache.
- **Atmospheric battlefield frame**: Four fixed far/near ash-cloud banks are rasterized once into a transparent Canvas layer, then reused behind gameplay and translated only by the existing far-depth impact parallax. Sparse warm crown highlights tie the cool masses to the sunset without competing with actions or explosions. A static semantic-CSS backplate extends the dusk, horizon, ember, and star roles through aspect-ratio gutters around the fitted stage. Neither layer animates at idle, accepts input, changes scroll geometry, or enters engine, replay, action-log, or Supabase state.
...
- **Material impact signatures**: Swept projectile collisions add optional local-deterministic `impactType` provenance to the explosion event. Terrain contact keeps dirt ejecta and adds a compact low thud; direct armor contact replaces dirt with bounded metal fragments and white-hot sparks, adds a `DIRECT HIT` readout, and layers a short metallic transient under the weapon boom. Reduced motion retains the readout, and multi-warhead audio coalesces to one armor-priority material cue per frame. Air/flight-cap events omit provenance and keep the legacy presentation. No field crosses the action log or Supabase contract.
...
- **Opening salvo assist**: During the match's opening seat rotation, the locally controlled human sees a static gold trajectory ribbon and cyan collision bracket computed from the real fixed-step launch, wind, configured gravity, terrain, tanks, and swept collision. After every seat has taken one turn—or whenever shield, airburst, or bounce behavior is selected—the renderer returns to the short launch-only guide. The cue is local presentation only, respects the existing `G` preference, adds no idle animation or layout, and never enters engine, replay, action-log, or Supabase state.
...
- **Armor-hit feedback**: A strict surviving, visible health drop spawns one coalesced owner-colored white-hot chassis flash plus four-to-ten bounded metal sparks. Repeated damage-over-time ticks refresh one burst per tank while distinct tanks remain independent; reduced motion retains the exact damage number but suppresses the decorative flash and sparks. Lethal and buried damage keep their existing numeric, K.O., and wreck feedback, and no armor-hit state crosses the action log or Supabase.
- **Turn-start wind gusts**: The first visible `PLAYER_TURN` frame for each `(round, turn)` may sweep five-to-eleven fixed procedural ribbons across the sky in the authoritative wind direction for exactly 48 rendered frames. Actual wind magnitude bounds ribbon count, length, speed, and alpha; near-calm and reduced-motion turns remain still, exact turn dedupe prevents retriggering, and idle rendering resumes after expiry. The cue is drawn behind terrain and never changes wind, projectile physics, replay, or Supabase state.
- **Pooled napalm firelight**: Consecutive authoritative fire columns share bounded additive elliptical blooms beneath the existing flame tongues. Broad fields split every 32 columns, output is capped at eight strongest world-ordered pools, and remaining life controls finite radius and alpha, replacing per-column glow blocks with warmer full-field illumination and fewer draw calls. Invalid cells or terrain surfaces emit no bloom; fire damage, timing, replay, audio, idle policy, and Supabase remain unchanged.
- **Projectile motion streaks**: Every finite live payload moving at a readable speed receives one short velocity-aligned gradient ribbon behind its current position, including the first launch frame and newly split children before history accumulates. True velocity controls direction and bounded length/alpha while the existing weapon profile controls accent and payload-scaled width; stationary or malformed motion stays silent. The stateless cue is drawn after history and before the halo/silhouette and never changes physics, replay, or Supabase state.
- **Impact depth parallax**: Explosion recoil separates the existing scene into far atmosphere at `0.12×`, distant ridges and wind ribbons at `0.35×`, and the destructible battlefield at `1×` camera displacement. The pure presentation profile bounds hostile finite offsets, malformed input fails closed, and zero displacement preserves exact rest geometry. Existing shake decay, reduced-motion suppression, oversized sky coverage, idle rendering, and the stationary HTML HUD remain unchanged; no camera state enters gameplay, replay, or Supabase.
