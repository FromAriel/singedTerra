...
**Current phase:** MVP0 + MVP1 + **MVP2 done** — networked play via **Supabase deterministic lockstep** is live and tested: room create/join/ready-up via Supabase Edge Functions, a Realtime action log (only `fire` actions committed, angle/power/weapon embedded), per-client deterministic `GameEngine` replay with seeded-PRNG wind, and flight regenerated locally from each `fire` action (no streaming). **Public/private lobbies, in-place recolor/rename, and room cleanup** shipped (`71c279c`), as did **heartbeat + lazy-GC reaping of dead rooms** (`a88e1a2`). A **pixel-bitmap terrain rewrite with gravity + tank burial** also shipped (`86f287b`). **Sprint 4 (Combat Depth)** has since shipped in full: the finite **ammo economy** (`{count, unlimited}` inventory with fire gating and decrement, no `Infinity` sentinel), the complete weapon roster (every weapon in [`SPEC.md`](SPEC.md) §4.5 is `implemented: true`, **Shield** included), the clickable **weapon strip** in the HUD, and **referee turn-enforcement** in `submit_action`. Later sprints landed on top: **Sprint 5**'s graphical overhaul (archived under [`archive/`](archive/)) and **Sprint 6**'s best-of-N match structure with a between-round shop. The project is now in an **ongoing review-and-polish phase** tracked in [`REVIEW_BACKLOG.md`](REVIEW_BACKLOG.md).
...
- [x] Weapon shop: a deterministic in-engine credit economy — tanks start with `STARTING_CREDITS`, earn `CREDITS_PER_DAMAGE`/point of damage dealt + a flat per-shot stipend, and spend via a turn-neutral `buy` action (engine + network log + `submit_action`, which skips the turn-cursor for buys). Prices/bundles mapped from the canonical Scorched Earth 1991 catalog (`docs/reference/`). Store panel UI in the HUD (credits badge + buy buttons, affordability-gated). Covered by the `store` harness (10 total). Networked buy deployed. Round-based shop (buy *between* rounds) **DONE** (Sprint 6 Slice 4 — the `ROUND_OVER` per-tank between-rounds shop, deployed).
...
- [x] **Fuel-limited movement:** fresh rounds grant 100 fuel; `A` / `D` and the
  fitted mobility rocker commit deterministic, terrain-aware eight-pixel moves
  through hot-seat and the ordered network action log. Movement is turn-neutral,
  spends only distance actually traveled, and stops at cliffs, bounds, tanks, or
  exhaustion. The arms-level-3 Fuel Tank accessory adds 100 current-round fuel.
  Covered by `movement.mjs`, client/input/HUD tests, Edge referee tests, and the
  production-browser layout matrix.
- [x] Shields: destructible particle force field — Sprint 4 Slice 3 DONE (engine absorption + `use_shield` action through the stack + referee turn-enforcement in `submit_action` + shield harness). Networked play deployed.
...
- [x] **Round system: best of N rounds, configurable** (Sprint 6 Slices 1+3). Deterministic auto-advance inside `resolve()`: a round end records the result (winner `roundWins++`), then ends the match (clinch at ⌈N/2⌉ wins) or regenerates the next round from a per-round derived seed, carrying credits/inventory/score and resetting health/terrain/positions. Configurable odd best-of-N via lobby Advanced settings. Covered by the `rounds` + `netrounds` harnesses. **Networked best-of-N DEPLOYED + merged (PR #7):** `next_round` and per-tank `ROUND_OVER` buys are logged through `submit_action` (with a `roundOver` flag that relaxes the shop turn-gate and re-seats the opener at a round boundary); `create_room` stores `rounds`. **Between-rounds shop DONE** (Slice 4): a `ROUND_OVER` pause stages the next round and opens a per-tank shop; a `next_round` action begins combat. **Remaining: a live 2-browser playtest to verify networked lockstep across the round boundary.**
- [x] **Scoreboard: damage dealt, kills, rounds won** (Sprint 6 Slice 2). Per-tank `kills` + `totalDamage` attributed to the shooter (overkill/self-damage excluded), accumulated across rounds; covered by the `scoreboard` harness. HUD shows a round indicator, per-player round-win pips, a round-transition banner, and a final scoreboard table in the game-over panel. **Per-match Postgres persistence DONE** (Sprint 6: `match_scores` table via migration `003` + `finish_game` upsert of the replay-derived scoreboard, deployed).
- [~] Supabase Postgres persistence: scores per session (session-keyed, no accounts) — reuses the MVP2 Supabase project (was SQLite). **Per-match foundation shipped** (`match_scores`, one row per match keyed by `room_id`); the session-level *aggregation/read* view (history across matches) is still TODO.
